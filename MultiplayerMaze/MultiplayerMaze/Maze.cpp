...
void Maze::Generate()
{
	...
	array_base_[height_mid][width_mid] = kFinish; //If use #define?
	...
	while (!wall_list_.empty())
	{
		...
		{
			if (array_base_[y_cur][x_cur] == kWall)
			{
				if (bPassable(y_cur, x_cur, dir_cur))
				{
					array_base_[y_cur][x_cur] = kEmpty;
					AddWall(y_cur, x_cur);
				}
			}
		}
		...
		{
			if ((num_exit_ - count_exit_cur) > 0)
			{
				//adjacency problem? 
				if (bSuitableExit(y_cur, x_cur))
				{
					array_base_[y_cur][x_cur] = kExit;
	...
	if (num_exit_ > count_exit_cur)
	{
		num_exit_ = count_exit_cur;
		cout << "The actual number of exits is : " << num_exit_ << endl;
	}
	UpdateArray();
	//Draw();
...
bool Maze::bPassable(const int y, const int x, const Direction dir) const
{
	switch (dir)
	{
	case kEast:
		if (array_base_[y][x + 1] != kWall || array_base_[y - 1][x] != kWall || array_base_[y + 1][x] != kWall)
		{
			return false;
		}
		break;
	case kNorth:
		if (array_base_[y - 1][x + 1] != kWall || array_base_[y][x - 1] != kWall || array_base_[y][x + 1] != kWall)
		{
			return false;
		}
		break;
	case kSouth:
		if (array_base_[y + 1][x] != kWall || array_base_[y][x - 1] != kWall || array_base_[y][x + 1] != kWall)
		{
			return false;
		}
		break;
...
{
	if (y - 1 >= 0 && (array_base_[y - 1][x] == kWall))
		wall_list_.push_back(Wall(y, x, Direction::kNorth));
	if (y + 1 <= (height_ - 1) && (array_base_[y + 1][x] == kWall))
		wall_list_.push_back(Wall(y, x, Direction::kSouth));
	if (x - 1 >= 0 && (array_base_[y][x - 1] == kWall))
		wall_list_.push_back(Wall(y, x, Direction::kWest));
	if (x + 1 <= (width_ - 1) && (array_base_[y][x + 1] == kWall))
...
void Maze::LoadFromFile(string name)
{
	...
	{
		cout << "Failed to load file, Please save a maze first." << endl;
	...
	if (in_file.is_open())
	{
		...
		int size = line.size();
		ReleaseMemory();
		array_base_ = new char* [size];
		for (int i = 0; i < size; i++)
		{
			array_base_[i] = new char[size];
		}
		for (int i = 0; i < size; i++)
		{
			array_base_[0][i] = line[i];
		}
		...
		for (int i = 1; i < size; i++)
		{
			...
			for (int j = 0; j < size; j++)
				...
				in_file.get(array_base_[i][j]);
			in_file.get();
		}
		Reset(height_, width_);
		UpdateArray();
		UpdateStartAndExit();
		//Draw();
	}
...
void Maze::UpdateStartAndExit()
{
	for (int i = 0; i < height_; i++)
	{
		for (int j = 0; j < width_; j++)
		{
			if (array_base_[i][j] == kExit) //kExit to kEntry
			{
				num_exit_++;
				list_exit_.push_back(new Point(i, j));
			}
			if (array_base_[i][j] == kFinish)
			{
				num_start_++;
...
bool Maze::MoveInTurns(bool b_show) //resize list_player_ timely?
{
...
bool Maze::bEndNormal() const
{
	return b_end_normal_;
}

void Maze::AutoOperate()
{
	b_end_normal_ = true;
	while (!MoveInTurns(false))
	{
	}
}
